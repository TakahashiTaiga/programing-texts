...
| D-FIN | 完了 | 通し確認 | — | — | 小 | 章立てとの一致（9章・46 節・150 項）・リンク（本の中の相対 104 件／アンカー 83 件）・解答の対応（83 問すべてに解説あり）を機械的に確認。囲み記事のラベル `重要：` 5箇所を style-guide の4種類に統一し、解答編の区切り見出しを他の3冊と同じ `### 演習` に統一。デプロイ／ポーリングの重複定義に初出への参照を付与し、`review-notes.md` に積まれていた章をまたぐ食い違い3件（4.6.3 の `CHOKIDAR_USEPOLLING`・3.2.1 のサイズ・6章末のサイズ）を修正。curriculum-map に完成の注記と「よくあるつまずき」列を追加。glossary に未登録の 15 語を追記。残りの申し送りは `review-notes.md` へ |
