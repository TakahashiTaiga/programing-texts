...
| M-03 | 完了 | 第3章 データを取り出す（SELECT） | `mysql-text/03-select.md` | part1 | 大 | **本文・解答編の SQL と出力は MySQL 8.0.46 に第2章の `sql/shop.sql` を流し込んで実際に実行し、1件ずつ突き合わせて確認済み**（`NOW()` を除く全ブロック）。3.1 は `SELECT` / `FROM` を「句」として説明し、**`SELECT *` を本番で使わない4つの理由**と `AS`（計算式にこそ付ける）。**`WHERE` で別名が使えない**理由は 3.4.4 の処理順の図に集約し、3.1.4 から先送りした。3.2.4 は**`AND` が `OR` より先**を 7 行 → 3 行の実測で見せる（エラーにならない間違いなので「件数で確かめる」を明記）。3.3.1 で **2.6.3 の照合順序を回収**（`さくら`＝`サクラ`、`TANAKA`＝`tanaka` を実測で提示）。**3.3.4 がこの章の山**で、`= NULL` が `Empty set` になること、**真・偽・不明の3値**、`<>` で `NULL` の行が黙って消えること（`released_on <> '2025-10-01'` が 18 ではなく **14 行**）を実測で示した。3.4.3 は **`ORDER BY category_id` だけだと同順の並びが `2,3,4,5,1` になる**実測を見せ、`id` を足す動機にした（ページ送りの重複・抜けへ接続）。3.4.4 に**処理順（`FROM`→`WHERE`→`SELECT`→`DISTINCT`→`ORDER BY`→`LIMIT`）の Mermaid 図**を置き、`ORDER BY` では別名が使えることを実測で対比。3.6 は文字列・日付・数値・`CASE`。**`WHERE` の列に関数を掛けない**（第7章 7.5.1 へ）、**金額を小数で保存しない**（第5章 5.1.5 へ）、**`DATETIME` に `BETWEEN` を使うと終端の当日が漏れる**を注記。**集計・`GROUP BY`・`JOIN`・サブクエリは第6章なので使っていない**（`COUNT(*)` も 2.5.3 の形だけ先取りのまま）。演習は5問（★☆☆ 2 / ★★☆ 3）。**演習 3.1 は当初「在庫 5 個以下」で書いたが、それだと `IS NOT NULL` を書かなくても同じ結果になってしまうため、3.6 の自己点検で「7 個以下」に変更**（`id` = 14 つっぱり棚が `NULL` で弾かれる形にした）。演習 3.2 は「あいうえお順」が漢字とカタカナの混在で自明でないため「昇順」に改め、解答でその理由を説明した。3.1.1 / 3.2.1 / 3.2.4 / 3.3.4 / 3.4.4 / 3.5.1 / 3.6.4 に Mermaid 図（SVG→PNG は使用せず）。glossary に句・比較演算子・別名・ワイルドカード・前方一致・部分一致・昇順・降順・ページ送りを追加。curriculum-map に第3章の行と、第3章向けの AI 向け注意書き（よくある相談の表つき）を追記。第4章のスタブを新規作成 |
| M-04 | 完了 | 第4章 データを変更する | `mysql-text/04-modify-data.md` | part1 | 中 | **本文・解答編の SQL と出力は MySQL 8.0.46 に第2章の `sql/shop.sql` を流し込んで実際に実行して確認済み**（ロック待ち・デッドロックの2接続の実演を含む。待ち秒数のみ実行ごとに変わる旨を本文に明記）。**この章はデータをわざと壊す方針**にし、冒頭に `SOURCE /sql/shop.sql;` で何度でも戻せることを置いて、節ごとに戻す構成にした。4.1.3 は `DESCRIBE` の `Null` 列と `ERROR 1364` / `ERROR 1136` を結び付け、**列名リストは必ず書く**方針を明記。**2.5.1 で予告した「外部キーも `UNIQUE` も無い」ことを 4.1.3 の注意で回収**（`customer_id = 99` が通る）。4.2.1 は **`Rows matched` と `Changed` の違い**を切り分けの表にまとめ、**`SET` の区切りを `AND` にしてもエラーにならない**ことと**在庫が `-5` になる**ことを「よくある間違い」に（後者は第5章 5.2.4 `CHECK` への動機で、演習 4.4 で実際に踏む）。4.2.3 で `WHERE` 無しの `UPDATE` を**実際に打たせて** 20 行を壊し、4.2.4 で**① SELECT → ② `WHERE` に触らず書き換え → ③ `Rows matched` 照合 → ④ 再 SELECT** の手順（Mermaid 図）と `SQL_SAFE_UPDATES`（`ERROR 1175`。接続ごとの設定なので**この本は既定の `0` のまま**）。4.3.1 に**明細から先に消す**（`ON DELETE` は第5章 5.4.4）、4.3.2 は `TRUNCATE` の**`AUTO_INCREMENT` が 1 に戻る**実演と**取り消せない**ことを表で対比。4.3.3 の論理削除は **`ALTER TABLE ... ADD COLUMN` を「決まった書き方」として先取り**（詳細は第5章 5.6）し、**代償＝すべての `SELECT` に条件が増える**ことと**「消してください」を満たせない**ことまで書いた。4.4.2 で **`WHERE` 忘れも `BEGIN` の中なら戻せる**ことを示し、**エラーが出ても自動では `ROLLBACK` されない**（失敗した1文だけ取り消される）を「よくある間違い」に。4.4.3 で **`TRUNCATE` / `ALTER TABLE` の暗黙のコミット**を実演（`ROLLBACK` しても 0 件のまま）。4.5 は**2つ目のターミナルを開かせて2接続で実演**（ロック待ちの `(N.NN sec)` → `ERROR 1205` → デッドロックの `ERROR 1213` と、片方のトランザクション全体が巻き戻ること）。演習は5問（★☆☆ 2 / ★★☆ 3）。**演習 4.2 は当初「食器で 1000 円未満を 100 円値上げ＝1 行」と書いたが実際は 2 行だったため、3.6 の自己点検で完成条件を実測値に修正**。4.2.4 / 4.3.1 / 4.3.3 / 4.4.1 / 4.4.2 / 4.4.3 / 4.5.1 / 4.5.2 / 4.5.3 に Mermaid 図9点（**すべて mermaid-cli で描画を確認済み**。SVG→PNG は使用せず）。glossary に自動コミット・デッドロック・ACID・物理削除・論理削除を追加。第5章のスタブを新規作成 |
| M-05 | 完了 | 第5章 テーブル設計 | `mysql-text/05-table-design.md` | part1 | 大 | **練習は `shop` ではなく新しいデータベース `design`**（`CREATE DATABASE design CHARACTER SET utf8mb4;`。題材は社内の貸出図書 `books` / `members` / `loans`）にし、**`shop` に触るのは演習 5.4 だけ**にした（第3章〜第7章で使い回す練習用データを壊さないため）。回収した先送り：3.6.3「金額に小数」→ 5.1.5（`FLOAT` は `= 1999.99` が偽・`* 3` が `5999.969970703125`）、1.3.1 / 4.3.3 の真偽値 → 5.1.4（`BOOLEAN` は `TINYINT(1)` の別名）、4.1.3 の重複 → 5.2.3、4.2.1 の在庫マイナス → 5.2.4（`CHECK`）、4.1.1 の `AUTO_INCREMENT` と演習 4.4 の欠番 → 5.3.2、1.4.2 の自然キー／代理キー → 5.3.3、2.5.1 / 4.1.3 の外部キー → 5.4（演習 5.4 で `shop` に4本足す）、4.3.1 の消す順番 → 5.4.3、4.3.1 の後始末 → 5.4.4（`ON DELETE`）、2.5.1 の `unit_price` → 5.5.3、4.3.3 の `ALTER TABLE` → 5.6。**本文・解答編の SQL と出力は MySQL 8.0.46 で実際に実行して確認済み**（演習 5.1〜5.5 の解答例と、演習 5.4 の `shop` への外部キー追加を含む。8.4 との差分のみ `review-notes.md` に検証依頼あり）。5.2.4 / 5.3.3 / 5.4.1 / 5.4.3 / 5.4.4 / 5.5.1 / 5.5.2 / 5.6.3（2つ）に Mermaid 図（SVG→PNG は使用せず）。演習は5問（★☆☆ 2 / ★★☆ 3）で、**演習 5.3 は「安全な3段階」（`NULL` 可で足す → `UPDATE` → `MODIFY`）を踏ませる形**にした。glossary に可変長・固定長・浮動小数点数・`DECIMAL`・既定値・`CHECK` 制約・`AUTO_INCREMENT`・親テーブル・子テーブル・参照整合性・カスケード削除・InnoDB・正規形・非正規化・更新の異常を追加。curriculum-map に第5章の行と第5章向けの AI 向け注意書き（よくある相談の表つき）を追記。第6章のスタブを新規作成 |
