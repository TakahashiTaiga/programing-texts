...
- [x] **`docker-text` 4.6.3 の補足の修正（D-06 で判明した誤り）** — **`D-FIN` で対応済み。**
      4.6.3 の補足から `CHOKIDAR_USEPOLLING` の記述を削り、
      「Vite は環境変数ではなく設定ファイルで指定するので、対処は 6.4.2 で扱う」に書き換えました
...
- [x] **`docker-text` 3.2.1 の補足のサイズ表記（D-07 で判明）** — **`D-FIN` で対応済み。**
      3.2.1 を「約 1.6 GB」に直し、7.3.1 の実測値と揃えました
...
- [x] **`docker-text` 第6章の「次の章へ」の記述（D-07 で判明）** — **`D-FIN` で対応済み。**
      「2つ合わせて 900 MB 前後、`db` を含めると 2 GB 前後」に直し、7.1.1 と揃えました
...
### `D-FIN`（docker-text 通し確認）で見つかったもの

**修正したもの**

| 箇所 | 内容 |
|------|------|
| 8.1.3 / 8.2.1 / 8.2.3 / 8.3.3 と解答編 第8章 | 囲み記事のラベル `> **重要：〜**` を、`docs/style-guide.md` 3.3 の4種類（よくある間違い／補足／注意／つまずいたら）に統一。データが消える話は `注意：`、読み飛ばしてよい背景は `補足：` に振り分けた（`重要：` は docker-text だけで使われていた） |
| 解答編 演習 8.1 | 解答に `**解説**` が無かったので追記（RUNBOOK 3.4「すべての解答に解説を付ける」） |
| 解答編 全体 | 章ごとの区切り見出し `### 演習問題` を、他の3冊と同じ `### 演習` に統一（8箇所） |
| 7.2.2 | `powershell` のコードブロックに `**Windows（PowerShell）**` の見出しが抜けていたので追加 |
| 7.5.3 のデプロイ | 1.4.3 と定義文が食い違っていたので、用語集の定義に合わせ、1.4.3 への参照を付けた |
| 6.4.2 のポーリング | 4.6.3 の定義の再掲だったので、4.6.3 への参照を付けた |
| 3.2.1 / 4.6.3 / 6章末 | 上の「内容に自信がない・要検討」に積まれていた3件（D-06 / D-07 で判明した章をまたぐ食い違い）を修正 |
| `ai/curriculum-map.md`（**docker-text 以外**） | fastapi-text の第5章・第9章の行が、バッククォート内の未エスケープの `\|`（`` `\| None` `` と `` `FIELD_LABELS[field] \|\| field` ``）で**表として崩れていた**ので、`\|` に直した。`D-FIN` の担当範囲外だが、`main` を取り込んだ際に検出した1行の描画バグのため、このPRで直した（`P-FIN` が python-text で直したものと同じ種類） |
| `docs/glossary.md` | 未登録だった 15 語を追記（マルチステージビルド／ビルド引数（`ARG`）／`slim`／Alpine Linux／glibc／musl／ホイール（wheel）／脆弱性／プロジェクト名（Compose）／起動ポリシー（`restart`）／シェルスクリプト／マネージドデータベース ほか） |

**修正しなかったもの（判断の記録）**

- [ ] `docker-text` 解答編 演習 6.4 の「サーバ」「ユーザ名」
      → **Adminer のログイン画面に出る文言そのまま**です。`docs/style-guide.md` 2.1 の補足
      （画面に出る文言は表示どおりに書く）に従い、長音を足していません
- [ ] `docker-text` 2.2.1〜2.2.2（WSL2・BIOS）と 2.2.3（Apple Silicon）に、片方の OS の手順しかない
      → **OS 固有のトラブルを扱う項**なので、これで正しい形です。
      それ以外の手順は Windows / macOS を並記済みです
- [ ] `docker-text` 第0章に理解度チェックと演習問題が無い
      → コードを書かない章のため、react-text 第0章・python-text 第0章・fastapi-text 第0章と同じ扱いです

**機械的に確認して問題が無かったもの**

- README の章立てと本文の見出し（9章・46 節・150 項）が完全に一致。`####` 以下の見出しは 0 件
- 本の中の相対リンク 104 件・アンカー 83 件が、すべて実在する見出しを指している
- 理解度チェック 52 問・演習 31 問（計 83 問）のすべてに解答があり、すべてに解説が付いている
- 解答編にあって本文に無い演習・本文にあって解答編に無い演習は 0 件
- コードブロックの言語指定漏れ 0 件（4連バッククォートの入れ子を含めて確認）。
  本文からの `.svg` の直接参照 0 件。`TODO(review-notes)` の残り 0 件
- 第0章から第8章まで「次の章へ」が途切れず、最後は mysql-text に接続している
- `docker-text/config.yaml` の `chapters`（10 件）と実ファイルが一致
- 「簡単です」「すぐできます」「ご存知のとおり」などの禁止表現は 0 件
- 長音の表記ゆれ（サーバ／ブラウザー／ユーザ など）は、上記の画面文言 3 箇所を除いて 0 件
