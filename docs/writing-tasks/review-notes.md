...
- [ ] `mysql-text` 第3章（M-03）**MySQL 8.4 での再確認**
  - **本文・解答編に載せた SQL と実行結果は、MySQL 8.0.46（Ubuntu パッケージ）に
    第2章の `sql/shop.sql` を流し込んで実際に実行し、1ブロックずつ突き合わせて確認済み**です
    （罫線の幅・値・`N rows in set` の件数まで、スクリプトで機械的に照合しました）。
    **確認できていないのは、本のターゲットである MySQL 8.4 との差分だけです**
  - [ ] 第3章で扱う範囲（`SELECT` / `WHERE` / `LIKE` / `IN` / `BETWEEN` / `IS NULL` /
        `ORDER BY` / `LIMIT` / `DISTINCT` / 文字列・日付・数値関数 / `CASE`）に、
        **8.0 と 8.4 で表示や挙動の差が無いこと**を一度確認してください。
        差が出そうなのは `ERROR 1054` / `ERROR 1064` の**メッセージ文言**です
  - [ ] 3.6.2 の `SELECT CURDATE(), NOW();` の結果は `2026-09-14` 時点のものです。
        本文にも「実行した日時によって変わります」と書いていますが、
        **`DATEDIFF('2026-09-14', ...)` を使った例と演習 3.5 の期待値（`315` など）は
        日付を固定してある**ので、こちらは何年後に読んでも変わりません
  - [ ] 3.4.3 で「`ORDER BY category_id` だけだと同じ分類の中が `2,3,4,5,1` の順になる」
        と実測値を載せています。**この並びは保証されたものではなく、環境や版で変わります。**
        本文でも「決まらない」ことを主旨として書いていますが、
        実機で別の並びになった場合は、**出力を差し替えるか「一例です」と注記を足してください**

- [ ] `mysql-text` 第4章（M-04）**MySQL 8.4 での再確認と、2接続の実演**
  - **本文・解答編に載せた SQL と実行結果は、MySQL 8.0.46（Ubuntu パッケージ）に
    第2章の `sql/shop.sql` を流し込んで実際に実行して確認済み**です。
    `Query OK, N rows affected` / `Rows matched` / `Changed` / `Records` の各行、
    エラー番号と文言（`1054` / `1136` / `1175` / `1205` / `1213` / `1292` / `1364` / `1366`）、
    罫線付きの表の中身まで、実機の出力を貼っています
  - [ ] **確認できていないのは、本のターゲットである MySQL 8.4 との差分だけ**です。
        とくに次の2つの文言に差が無いかを見てください
        - 4.2.4 の `ERROR 1175`（セーフアップデートモード）。
          **クライアントによっては末尾に「To disable safe mode, ...」が続く形**になります
        - 4.4.3 の `SET autocommit = 0;` / `SELECT @@autocommit;` の表示
  - [ ] **4.5（ロック）は、2つの `mysql>` を同時に開いて実演する節です。**
        検証は Docker（`docker compose exec db mysql ...`）を2つ開いて行ってください。
        本文の手順どおりに**画面が実際に止まること**、
        `COMMIT` を打った瞬間に**もう一方が動き出すこと**を確認してほしいところです
  - [ ] 4.5.1 の接続 B の実行結果に **`Query OK, 1 row affected (23.45 sec)`** と書いています。
        **この秒数は「読者が `COMMIT` を打つまでの時間」なので、実行のたびに変わります。**
        本文にもその旨を明記していますが、値そのものは代表例です
  - [ ] 4.5.1 の「既定では **50 秒**で諦めます」は `innodb_lock_wait_timeout` の既定値です。
        8.4 でも 50 のままかを確認してください
  - [ ] 4.5.2 のデッドロックは、**どちらの接続が犠牲になるかが MySQL の判断**です。
        本文では接続 B に `ERROR 1213` が出る前提で書いていますが、
        **逆になる可能性があります。** 実機で逆になった場合は、
        本文に「どちらが犠牲になるかは MySQL が決めます」という注記を足してください
  - [ ] 4.5.1 / 4.5.2 で「**新しいターミナルを開く**」手順（PowerShell の新規ウィンドウ、
        macOS の `Command` + `T`、VS Code のターミナルパネルの `+`）は**実機未確認**です
  - [ ] 4.3.3 と演習 4.5 で `ALTER TABLE ... ADD COLUMN` を、
        **第5章 5.6 の先取りとして「決まった書き方」**の扱いで使っています。
        第5章を書くときに、**ここで説明済みの範囲と重複しないか**を見てください

- [ ] `mysql-text` 第5章（M-05）**MySQL 8.4 での再確認**
  - **本文・解答編に載せた SQL と実行結果は、MySQL 8.0.46（Ubuntu パッケージ）で
    実際に実行して確認済み**です（第2章〜第4章と同じ環境）。
    `DESCRIBE` / `SHOW CREATE TABLE` の表示、エラー番号と文言
    （`1048` / `1062` / `1101` / `1264` / `1265` / `1292` / `1364` / `1406` / `1451` / `1452` / `3819`）、
    `FLOAT` の計算結果（`5999.969970703125`）、`AUTO_INCREMENT` の欠番まで、実機の出力を貼っています。
    **演習 5.1〜5.5 の解答例も、すべて通しで実行して確認済み**です
    （演習 5.4 は第2章の `sql/shop.sql` を流し込んだ `shop` に対して実行）
  - [ ] **確認できていないのは、本のターゲットである MySQL 8.4 との差分だけ**です。
        とくに次の3点を見てください
        - `SHOW CREATE TABLE books` で **`CHECK` 制約が名前のアルファベット順に並ぶ**こと
          （本文は `chk_books_copies` → `chk_books_price` の順で載せています）
        - 5.4.4 の `ALTER TABLE ... ADD CONSTRAINT` の **`N rows affected`**
          （本文には「手元と違って構わない」という補足を入れてあります）
        - `registered_at` の `Extra` 列の **`DEFAULT_GENERATED`** の表示
  - [ ] 5.3.2 の `LAST_INSERT_ID()` が返す番号（本文は `5`）は、
        **そこまでに出したエラーの回数で変わります**（本文に補足あり）。
        章を頭から順に打った場合は `5` になることを確認済みです
  - [ ] 4.3.3 / 演習 4.5 で先取りしていた `ALTER TABLE ... ADD COLUMN` は、
        5.6.2 で改めて体系的に扱いました（M-04 からの申し送りへの対応）。
        **重複はしていないはずですが、通し読みで確認してください**
  - [ ] 5.6.3 の「本番での `ALTER TABLE`」は、**大きなテーブルでの実測ではありません**
        （練習環境の数行のテーブルでは再現できないため、一般的な注意として書いています）。
        「数分から数時間」という目安の表現が強すぎないか、レビューで見てください

> **補足：`mysql-text/90-answers-part1.md` の冒頭から `./91-answers-part2.md` への
> リンクがありますが、このファイルはまだ存在しません**（第6章 M-06 で作成予定）。
> M-01 の時点から入っているもので、M-06 が終われば解消します。
