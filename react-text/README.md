# プログラミング未経験から始める React

**このテキスト集の1冊目です。**
プログラミングを一度もやったことがない人が、Web の仕組みから始めて
React でアプリを作れるようになるまでを扱います。

---

## この本を読む前に

**まず [第0章](./00-introduction.md) を読んで、AI サポートの準備をしてください。**
環境構築やエラーで止まらないための、いちばん大事な準備です。

---

## 目次

### [第0章 はじめに](./00-introduction.md)

- 0.1 このテキストについて
  - 0.1.1 対象読者
  - 0.1.2 このテキストを終えると何ができるようになるか
  - 0.1.3 全5冊の中での位置づけ
- 0.2 AI サポートの準備
  - 0.2.1 なぜ AI を使うのか
  - 0.2.2 使う AI を決める
  - 0.2.3 指示ファイルを読み込ませる
  - 0.2.4 動作確認
- 0.3 学習の進め方
  - 0.3.1 必ず手を動かす
  - 0.3.2 詰まったときの動き方
  - 0.3.3 理解度チェックと演習問題
  - 0.3.4 ペースの目安
- 0.4 このテキストの表記ルール

### [第1章 Web の仕組みと開発環境](./01-web-and-environment.md)

- 1.1 プログラムとは何か
  - 1.1.1 コンピュータは指示どおりにしか動かない
  - 1.1.2 プログラミング言語とは
  - 1.1.3 このテキストで使う言語
- 1.2 Web ページが表示される仕組み
  - 1.2.1 ブラウザとサーバー
  - 1.2.2 リクエストとレスポンス
  - 1.2.3 URL の読み方
  - 1.2.4 HTML・CSS・JavaScript の役割分担
- 1.3 エディタを用意する
  - 1.3.1 なぜ Word ではだめなのか
  - 1.3.2 VS Code をインストールする
  - 1.3.3 日本語化する
  - 1.3.4 拡張子を表示する（Windows は必須）
- 1.4 ターミナルに慣れる
  - 1.4.1 ターミナルとは
  - 1.4.2 ターミナルを開く
  - 1.4.3 いまどこにいるか確認する
  - 1.4.4 移動する・一覧を見る
  - 1.4.5 作業用ディレクトリを作る
  - 1.4.6 ターミナルでよくあるエラー
- 1.5 Node.js を用意する
  - 1.5.1 Node.js とは
  - 1.5.2 インストールする
  - 1.5.3 動作確認する
  - 1.5.4 うまくいかないとき
- 1.6 はじめての Web ページ
  - 1.6.1 ファイルを作る
  - 1.6.2 ブラウザで開く
  - 1.6.3 書き換えて反映を確かめる
  - 1.6.4 開発者ツールをのぞいてみる

### [第2章 HTML — 情報の骨組み](./02-html.md)

- 2.1 HTML の基本
  - 2.1.1 タグと要素
  - 2.1.2 HTML ファイルの骨組み
  - 2.1.3 入れ子構造
  - 2.1.4 コメント
- 2.2 文章を組み立てるタグ
  - 2.2.1 見出し
  - 2.2.2 段落と改行
  - 2.2.3 リスト
  - 2.2.4 強調
- 2.3 属性
  - 2.3.1 属性の書き方
  - 2.3.2 リンク
  - 2.3.3 画像
  - 2.3.4 相対パスと絶対パス
  - 2.3.5 id と class
- 2.4 表とフォーム
  - 2.4.1 表
  - 2.4.2 フォームの基本
  - 2.4.3 いろいろな入力欄
  - 2.4.4 ボタン
- 2.5 意味のある構造をつくる
  - 2.5.1 div と span
  - 2.5.2 header / main / footer / section
  - 2.5.3 なぜ意味を持たせるのか

### [第3章 CSS — 見た目を整える](./03-css.md)

- 3.1 CSS の書き方
  - 3.1.1 CSS とは
  - 3.1.2 HTML から CSS を読み込む
  - 3.1.3 ルールの構造
- 3.2 セレクタ
  - 3.2.1 要素セレクタ
  - 3.2.2 class セレクタ
  - 3.2.3 id セレクタ
  - 3.2.4 子孫セレクタと複数指定
  - 3.2.5 詳細度（どれが優先されるか）
- 3.3 ボックスモデル
  - 3.3.1 すべての要素は箱
  - 3.3.2 padding / border / margin
  - 3.3.3 width と box-sizing
  - 3.3.4 margin の相殺
- 3.4 文字と色
  - 3.4.1 色の指定方法
  - 3.4.2 フォント
  - 3.4.3 行間・字間・揃え
- 3.5 レイアウト
  - 3.5.1 display の基本
  - 3.5.2 Flexbox の考え方
  - 3.5.3 主軸方向の配置（justify-content）
  - 3.5.4 交差軸方向の配置（align-items）
  - 3.5.5 折り返しと隙間
  - 3.5.6 Grid にざっと触れる
- 3.6 レスポンシブ対応
  - 3.6.1 スマホで見るとどうなるか
  - 3.6.2 viewport の設定
  - 3.6.3 メディアクエリ
- 3.7 CSS が効かないときの調べ方

### [第4章 JavaScript の基礎（前半）](./04-javascript-basics.md)

- 4.1 JavaScript を動かす
  - 4.1.1 コンソールで試す
  - 4.1.2 HTML から読み込む
  - 4.1.3 `console.log` で値を確認する
  - 4.1.4 エラーメッセージの読み方
- 4.2 変数
  - 4.2.1 変数とは
  - 4.2.2 `let` と `const`
  - 4.2.3 変数の名前の付け方
  - 4.2.4 `var` を使わない理由
- 4.3 データ型と演算子
  - 4.3.1 数値
  - 4.3.2 算術演算子
  - 4.3.3 文字列とテンプレートリテラル
  - 4.3.4 真偽値
  - 4.3.5 `undefined` と `null`
  - 4.3.6 比較演算子と `===`
  - 4.3.7 論理演算子
  - 4.3.8 型の自動変換に注意する
- 4.4 条件分岐
  - 4.4.1 `if`
  - 4.4.2 `else` と `else if`
  - 4.4.3 条件を組み合わせる
  - 4.4.4 三項演算子
  - 4.4.5 `switch`
- 4.5 繰り返し
  - 4.5.1 `for`
  - 4.5.2 `while`
  - 4.5.3 `break` と `continue`
  - 4.5.4 繰り返しの中で条件を使う
- 4.6 関数
  - 4.6.1 関数とは
  - 4.6.2 引数と戻り値
  - 4.6.3 アロー関数
  - 4.6.4 スコープ
  - 4.6.5 関数を分ける基準

### [第5章 JavaScript の基礎（後半）](./05-javascript-advanced.md)

- 5.1 配列
  - 5.1.1 配列とは
  - 5.1.2 要素の取り出しと書き換え
  - 5.1.3 追加と削除
  - 5.1.4 配列を繰り返す
  - 5.1.5 よく使うメソッド
- 5.2 オブジェクト
  - 5.2.1 オブジェクトとは
  - 5.2.2 プロパティの読み書き
  - 5.2.3 入れ子のオブジェクト
  - 5.2.4 オブジェクトの配列
  - 5.2.5 `undefined` エラーとの付き合い方
- 5.3 配列を扱う便利な関数
  - 5.3.1 `map` — 全部を変換する
  - 5.3.2 `filter` — 条件に合うものだけ取り出す
  - 5.3.3 `find` — 最初の1つを取り出す
  - 5.3.4 `reduce` — 1つの値にまとめる
  - 5.3.5 `sort` — 並べ替える
  - 5.3.6 つなげて使う
- 5.4 分割代入とスプレッド構文
  - 5.4.1 分割代入
  - 5.4.2 スプレッド構文
  - 5.4.3 イミュータブルな更新の考え方
- 5.5 非同期処理
  - 5.5.1 同期と非同期
  - 5.5.2 `setTimeout` で体感する
  - 5.5.3 `Promise`
  - 5.5.4 `async` / `await`
  - 5.5.5 `fetch` でデータを取得する
  - 5.5.6 エラー処理
- 5.6 モジュール
  - 5.6.1 ファイルを分ける理由
  - 5.6.2 `export` と `import`
  - 5.6.3 デフォルトエクスポート
  - 5.6.4 `type="module"` の注意点
- 5.7 DOM 操作とイベント
  - 5.7.1 DOM とは
  - 5.7.2 要素を取得する
  - 5.7.3 内容とスタイルを変える
  - 5.7.4 イベントを受け取る
  - 5.7.5 フォームの入力値を取得する
  - 5.7.6 要素を追加・削除する
  - 5.7.7 なぜこれが大変なのか

### [第6章 React をはじめる](./06-react-start.md)

- 6.1 React が解決する問題
  - 6.1.1 DOM 操作の何が大変だったか
  - 6.1.2 「状態から画面を作る」という発想
  - 6.1.3 React・Vue・Angular
- 6.2 Vite でプロジェクトを作る
  - 6.2.1 Vite とは
  - 6.2.2 プロジェクトを作成する
  - 6.2.3 開発サーバーを起動する
  - 6.2.4 よくあるエラーと対処
- 6.3 プロジェクトの中身を読む
  - 6.3.1 ディレクトリ構成
  - 6.3.2 `index.html` と `main.jsx`
  - 6.3.3 `App.jsx`
  - 6.3.4 `package.json` と `node_modules`
  - 6.3.5 CSS の置き場所
- 6.4 JSX
  - 6.4.1 JSX とは
  - 6.4.2 HTML との違い
  - 6.4.3 波かっこで値を埋め込む
  - 6.4.4 属性に値を渡す
  - 6.4.5 フラグメント
- 6.5 コンポーネント
  - 6.5.1 コンポーネントとは
  - 6.5.2 コンポーネントを作る
  - 6.5.3 ファイルを分ける
  - 6.5.4 コンポーネントの分け方の基準
  - 6.5.5 命名規則

### [第7章 props と state](./07-props-and-state.md)

- 7.1 props — 部品に値を渡す
  - 7.1.1 props とは
  - 7.1.2 props を受け取る
  - 7.1.3 分割代入で受け取る
  - 7.1.4 いろいろな型を渡す
  - 7.1.5 `children`
  - 7.1.6 props は書き換えられない
- 7.2 state — 変化する値を持つ
  - 7.2.1 なぜ普通の変数ではだめなのか
  - 7.2.2 `useState` の使い方
  - 7.2.3 state を更新する
  - 7.2.4 state を直接書き換えてはいけない理由
  - 7.2.5 前の値をもとに更新する
  - 7.2.6 state に配列やオブジェクトを入れる
- 7.3 イベントを扱う
  - 7.3.1 `onClick`
  - 7.3.2 関数を渡すのと呼び出すのの違い
  - 7.3.3 引数を渡す
  - 7.3.4 いろいろなイベント
  - 7.3.5 イベントオブジェクト
- 7.4 リストを表示する
  - 7.4.1 `map` で要素を並べる
  - 7.4.2 `key` が必要な理由
  - 7.4.3 `key` に index を使ってはいけない場面
  - 7.4.4 リストに要素を追加・削除する
- 7.5 条件によって表示を変える
  - 7.5.1 `&&` を使う
  - 7.5.2 三項演算子を使う
  - 7.5.3 早期 return
  - 7.5.4 `0` が表示されてしまう罠
- 7.6 フォームを扱う
  - 7.6.1 制御コンポーネント
  - 7.6.2 `value` と `onChange`
  - 7.6.3 複数の入力欄をまとめて扱う
  - 7.6.4 チェックボックスとセレクト
  - 7.6.5 送信時の `preventDefault`

### [第8章 状態設計と副作用](./08-state-design-and-effects.md)

- 8.1 状態のリフトアップ
  - 8.1.1 兄弟コンポーネント間で値を共有したい
  - 8.1.2 共通の親に state を上げる
  - 8.1.3 更新関数を props で渡す
  - 8.1.4 state はどこに置くべきか
  - 8.1.5 持たなくていい state を見分ける
- 8.2 useEffect — 描画以外の処理
  - 8.2.1 副作用とは
  - 8.2.2 `useEffect` の使い方
  - 8.2.3 依存配列
  - 8.2.4 無限ループが起きる仕組みと直し方
  - 8.2.5 クリーンアップ関数
  - 8.2.6 `useEffect` を使うべきでない場面
- 8.3 サーバーからデータを取得する
  - 8.3.1 `fetch` と `useEffect` を組み合わせる
  - 8.3.2 ローディング状態
  - 8.3.3 エラー状態
  - 8.3.4 3つの state
  - 8.3.5 CORS エラーに遭遇したら
- 8.4 useRef / useMemo / useCallback
  - 8.4.1 `useRef` — DOM を直接触る
  - 8.4.2 `useRef` — 再描画されない値を持つ
  - 8.4.3 `useMemo` — 計算結果を覚えておく
  - 8.4.4 `useCallback` — 関数を覚えておく
  - 8.4.5 最適化は測ってから
- 8.5 カスタムフック
  - 8.5.1 同じロジックが2箇所に出てきたら
  - 8.5.2 カスタムフックの作り方
  - 8.5.3 `useFetch` を作る
  - 8.5.4 フックのルール

### [第9章 ルーティングと全体設計](./09-routing-and-architecture.md)

- 9.1 React Router — 画面を切り替える
  - 9.1.1 SPA とは
  - 9.1.2 React Router をインストールする
  - 9.1.3 ルートを定義する
  - 9.1.4 `Link` で移動する
  - 9.1.5 URL パラメータ
  - 9.1.6 ネストしたルートと共通レイアウト
  - 9.1.7 404 ページ
- 9.2 Context — 離れた場所に値を届ける
  - 9.2.1 props のバケツリレー問題
  - 9.2.2 Context の作り方
  - 9.2.3 Provider と `useContext`
  - 9.2.4 Context を使いすぎない
  - 9.2.5 状態管理ライブラリの話
- 9.3 ディレクトリ構成の考え方
  - 9.3.1 ファイルが増えてくると
  - 9.3.2 種類で分ける vs 機能で分ける
  - 9.3.3 このテキストで採用する構成
  - 9.3.4 命名規則
- 9.4 エラー処理とローディング表示
  - 9.4.1 エラーバウンダリ
  - 9.4.2 ローディング表示の統一
  - 9.4.3 ユーザーに何を見せるか

### [第10章 実践：タスク管理アプリを作る](./10-practice-task-app.md)

- 10.1 作るものを決める
  - 10.1.1 完成イメージ
  - 10.1.2 機能の一覧を書き出す
  - 10.1.3 最初に作る範囲を決める
- 10.2 設計する
  - 10.2.1 画面を紙に描く
  - 10.2.2 コンポーネントに分解する
  - 10.2.3 データの形を決める
  - 10.2.4 state をどこに置くか決める
- 10.3 段階的に実装する
  - 10.3.1 プロジェクトを作る
  - 10.3.2 まず一覧を表示する
  - 10.3.3 タスクを追加する
  - 10.3.4 完了・未完了を切り替える
  - 10.3.5 タスクを削除する
  - 10.3.6 絞り込み
  - 10.3.7 並べ替え
- 10.4 保存する
  - 10.4.1 リロードすると消えてしまう問題
  - 10.4.2 `localStorage` とは
  - 10.4.3 保存と読み込み
  - 10.4.4 カスタムフックに切り出す
- 10.5 仕上げ
  - 10.5.1 見た目を整える
  - 10.5.2 空のときの表示
  - 10.5.3 入力チェック
  - 10.5.4 動作確認のチェックリスト
- 10.6 発展課題
  - 10.6.1 期限を付ける
  - 10.6.2 カテゴリ分け
  - 10.6.3 検索
  - 10.6.4 詳細ページ

### [第11章 次のステップ](./11-next-steps.md)

- 11.1 ここまでで身についたこと
  - 11.1.1 チェックリスト
  - 11.1.2 自分の実力を確認する
- 11.2 TypeScript
  - 11.2.1 型があると何が嬉しいか
  - 11.2.2 最小限の入門
  - 11.2.3 いつ学び始めるべきか
- 11.3 テストとデプロイ
  - 11.3.1 なぜテストを書くのか
  - 11.3.2 ビルドする
  - 11.3.3 公開する（Vercel / Netlify）
- 11.4 Git と GitHub
  - 11.4.1 なぜバージョン管理が必要か
  - 11.4.2 最低限のコマンド
- 11.5 次の本へ
  - 11.5.1 なぜ次は Python なのか
  - 11.5.2 学習を続けるコツ

### [解答編 その1（第1章〜第5章）](./90-answers-part1.md)
### [解答編 その2（第6章〜第11章）](./91-answers-part2.md)

---

## 執筆状況

**この本は全章の執筆が完了し、通し確認（`R-FIN`）まで終わっています。**

| 章 | 状態 |
|----|------|
| 第0章 | ✅ 完成 |
| 第1章 | ✅ 完成 |
| 第2章 | ✅ 完成 |
| 第3章 | ✅ 完成 |
| 第4章 | ✅ 完成 |
| 第5章 | ✅ 完成 |
| 第6章 | ✅ 完成 |
| 第7章〜第10章 | ✅ 完成 |
| 第11章 | ✅ 完成 |
| 解答編 | ✅ 完成（第1章〜第11章。理解度チェック 73 問・演習 47 問すべてに解答と解説あり） |
| 通し確認（`R-FIN`） | ✅ 完了 |

### 通し確認で見たこと（`R-FIN`）

| 項目 | 結果 |
|------|------|
| README の章立てと本文の節・項の一致 | ✅ 過不足なし（`X.Y.Z` の 3 階層、`####` 以下は不使用） |
| 章間リンク・解答編リンク | ✅ 相対リンク 111 件すべて到達可能。解答編へのリンクは全章アンカー付きに統一 |
| 理解度チェック・演習と解答の対応 | ✅ 全問に解答があり、すべてに解説が付いている |
| コードブロックの言語指定 | ✅ すべて指定あり |
| Windows / macOS の手順 | ✅ OS 依存のコマンドは両方を併記 |
| `config.yaml` の `chapters` と実ファイル | ✅ 一致（14 ファイル） |
| 用語の表記ゆれ | ✅ 修正済み（詳細は `docs/writing-tasks/review-notes.md`） |

**人間の対応が残っているもの**（スクリーンショット、実機検証、図解の追加候補）は
[`docs/writing-tasks/review-notes.md`](../docs/writing-tasks/review-notes.md) にまとめてあります。

---

## 更新履歴

| 日付 | 内容 |
|------|------|
| 2026-08 | 全11章＋解答編の執筆が完了。通し確認（`R-FIN`）を実施 |
