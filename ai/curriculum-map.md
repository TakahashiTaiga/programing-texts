...
| 3 | データを取り出す（SELECT） | 第2章の範囲に加えて：**`SELECT` の基本**（`SELECT` 句＝どの列 / `FROM` 句＝どのテーブル・**句**という語・`*` は全列・結果は一時的な表で元のテーブルは変わらない・**行の並びは指定しなければ決まらない**（1.3.2 の回収）・列は書いた順に出る・`Unknown column 'x' in 'field list'`＝`SELECT` 句での列名ミス）、**`SELECT *` を本番で使わない4つの理由**（転送量・列の増減で壊れる・列の順番依存・意図が読めない。速度の話は第7章 7.2）、**`AS` で別名**（計算式にこそ付ける・`price * 1.1` はテーブルに無い列・`AS` は省略可だがこの本では書く・空白入りは `` ` `` で囲む・**`WHERE` では別名が使えない**（`Unknown column ... in 'where clause'`。理由は 3.4.4））、**`WHERE`**（`SELECT` は列を選び `WHERE` は行を絞る・1行ずつ判定・**文字列と日付は `'` で囲み数値は囲まない**・引用符を忘れると列名と解釈される）、**比較演算子**（`=` `<>` `!=` `>` `>=` `<` `<=`・**等しいは `=` ひとつ**（Python の `==` / JS の `===` との違い。`UPDATE ... SET` でも同じ `=`）・日付は `'YYYY-MM-DD'` で比較できる）、**`AND` / `OR` / `NOT`**（`AND` で減り `OR` で増える・`NOT 列 = 値` は `<>` と同じ）、**優先順位**（**`AND` が `OR` より先**・`category_id = 1 OR category_id = 2 AND price >= 3000` が 7 行になる実演 → かっこを付けて 3 行・**エラーにならないので件数で確かめる**）、**`LIKE`**（**ワイルドカード** `%`＝0文字以上 / `_`＝ちょうど1文字・前方 / 後方 / 部分一致・**照合順序に従うので `さくら`＝`サクラ`、`TANAKA`＝`tanaka`**（2.6.3 の回収）・`=` に `%` を書くと `Empty set`・`%` 始まりは遅い（第7章 7.5.2））、**`IN`**（`OR` の並びを短く・数値でも可・**`NOT IN` は候補に `NULL` があると0件**）、**`BETWEEN A AND B`**（**両端を含む**・逆順に書くと `Empty set`・**`DATETIME` に使うと終端が `00:00:00` 扱いで当日が漏れる** → `< 翌日` と書く）、**`NULL`**（`= NULL` は `Empty set`・**条件は真・偽・不明の3値で、`WHERE` は真の行だけ残す**・`NULL = NULL` すら `NULL`・**`IS NULL` / `IS NOT NULL`**・**`<>` を書くと `NULL` の行が黙って消える**（`released_on <> '2025-10-01'` が 18 ではなく 14 行）→ `OR 列 IS NULL` で足す・**`0` と空文字と `NULL` は別物**）、**`ORDER BY`**（`WHERE` の後ろ・`ASC`（既定・省略可）/ `DESC`・**`DESC` と `DESCRIBE` は別物**（2.4.4）・**MySQL では `NULL` が昇順の先頭**・**同じ値の行の並びは決まらない**実演（`ORDER BY category_id` が `2,3,4,5,1`）→ **最後のキーに `id` を足す**（ページ送りでの重複・抜けを防ぐ））、**`LIMIT` / `OFFSET`**（上位◯件・**`ORDER BY` とセット**・`OFFSET`＝（ページ番号 − 1）× 件数・fastapi-text 第6章の `skip` / `limit` の正体）、**処理される順番**（**`FROM` → `WHERE` → `SELECT` → `DISTINCT` → `ORDER BY` → `LIMIT`**。書く順と違う・だから**別名は `WHERE` で使えず `ORDER BY` で使える**）、**`DISTINCT`**（`SELECT` の直後に1回・**行（列の組み合わせ）単位**で重複を見る・知らないテーブルの「その列が取りうる値」を調べる定番・**1列だけ重複除去はできない**（第6章 6.5.2 の `GROUP BY`）・`COUNT(DISTINCT ...)` は第6章 6.5）、**関数**（**引数**と**戻り値**・`SELECT` にも `WHERE` にも書ける・`FROM` 無しの `SELECT 関数(...)` で1行だけ試せる・**文字列**＝`CONCAT` / `CHAR_LENGTH`（文字数）と `LENGTH`（バイト数。日本語1文字＝3バイト。2.6.1 の回収）/ `UPPER` / `LOWER` / `SUBSTRING`（**位置は1から**。Python・JS の 0 始まりとの違い）/ `REPLACE` / `TRIM`、**日付**＝`CURDATE` / `NOW`（実行のたび変わる）/ `YEAR` `MONTH` `DAY` `HOUR` / `DATE` / `DATE_FORMAT`（`%Y %m %d %H %i`）/ `DATEDIFF(a, b)`＝a − b / `DATE_ADD(d, INTERVAL n 単位)` / `DATE_SUB`、**数値**＝`ROUND`（`ROUND(x, n)` も）/ `FLOOR` / `CEIL` / `ABS` / `MOD`・**`WHERE` の列に関数を掛けない**（`YEAR(released_on) = 2026` → `>= '2026-01-01' AND < '2027-01-01'`。理由は第7章 7.5.1）・**金額を小数で保存しない**（第5章 5.1.5））、**`CASE`**（`CASE WHEN 条件 THEN 値 ... ELSE ... END` と `CASE 列 WHEN 値 ...` の2形・**上から順に判定して最初に当てはまったところで止まる**（狭い条件を先に書く）・**`ELSE` が無いとどれにも当たらない行が `NULL`**・`END` 忘れは `ERROR 1064`） |
| 4 | データを変更する | 第3章の範囲に加えて：**`INSERT`**（`INSERT INTO 表 (列, ...) VALUES (値, ...)`・**列名の並びと値の並びが前から順に対応**・`id` は `AUTO_INCREMENT` なので書かない・文字列は `'` で囲む（囲み忘れ＝`ERROR 1054`、型違い＝`ERROR 1366`）・**`VALUES (...), (...)` で複数件を1回で**（`Records: N` の行が出る。1件ずつ打つのと違い「全部入るか1件も入らないか」）・**省略した列には `NULL`**（`NOT NULL` の列は省略できず `ERROR 1364`。省略可否は `DESCRIBE` の `Null` 列）・列名リストごと省略すると `DESCRIBE` の順に全列必要（数が合わないと `ERROR 1136`）・**この本では列名リストを必ず書く**）、**`UPDATE`**（`SET` が「何を」・`WHERE` が「どの行を」・**`Rows matched`（当たった行数）と `Changed`（実際に変わった行数）は別物**（`matched: 0`＝`WHERE` を疑う／`matched: 1, Changed: 0`＝`SET` を疑う）・当たらなくてもエラーは出ない・**`SET` の右側に式が書ける**（`stock = stock - 3`＝いまの値から引く）・**在庫がマイナスになってもエラーが出ない**（`CHECK` は第5章 5.2.4）・`SET` の区切りは `,`（**`AND` と書いてもエラーにならず `0` か `1` が入る**）・複数列を1回で更新すると「途中の状態」を見せずに済む）、**`WHERE` の書き忘れ**（全行に当たる。**警告も確認も出ず、元の値はどこにも残らない**。`shop.sql` で戻せるのは練習用データだけで、本物はバックアップだけが手段＝第8章 8.6）、**事故を防ぐ手順**（**① `SELECT` で当たる行を見る → ② `WHERE` に触らず `UPDATE` に書き換える → ③ `Rows matched` を ① と照合 → ④ もう一度 `SELECT`**・変わってほしくない行も一緒に表示する・**危ない操作の前に `BEGIN;`**・`SET SQL_SAFE_UPDATES = 1;` で主キーを使わない `UPDATE` / `DELETE` が `ERROR 1175` で止まる（**接続ごとの設定**。この本は既定の `0` で進める））、**`DELETE`**（`SET` が無い・当たらなくてもエラーは出ない・**注文を消すときは明細から先に**（逆だと孤立した明細が残る。`ON DELETE` は第5章 5.4.4））、**`TRUNCATE` との違い**（`WHERE` で絞れない・**いつも `0 rows affected`**・**`AUTO_INCREMENT` が 1 に戻る**（`DELETE` では欠番のまま続く）・**`ROLLBACK` で取り消せない**・「速い `DELETE`」ではなく、この本では練習用テーブルを空にするときだけ）、**論理削除**（物理削除＝行を消す／論理削除＝`deleted_at` に**日時**を入れて残す（真偽値にしないのは「いつ」が要るから。真偽値は第5章 5.1.4）・列の追加は `ALTER TABLE 表 ADD COLUMN 列 型;`（**書き方の詳細は第5章 5.6**。ここでは「決まった書き方」として先取り）・**代償は「すべての `SELECT` に `WHERE deleted_at IS NULL` が必要」**（書き忘れてもエラーが出ない）・行が減らないので検索は遅くなる（第7章）・**「消してください」という要望は満たせていない**）、**トランザクション**（**片方だけ成功した状態**（明細だけ／在庫だけ）をなくす仕組み・`BEGIN` で開始・`COMMIT` で確定・`ROLLBACK` で全部取り消し・自分の接続からは変更後が見える・**`WHERE` 忘れの `UPDATE` も `BEGIN` の中なら戻せる**・**エラーが出ても自動では取り消されない**（取り消されるのは失敗した1文だけ。トランザクションは開いたままなので自分で `ROLLBACK`）・**`ROLLBACK` しても `AUTO_INCREMENT` の番号は戻らない**）、**自動コミット**（`SELECT @@autocommit;` が `1` が既定・`@@` は MySQL 自身の設定値なので `FROM` 不要・`BEGIN` はこれを一時的に止める・`SET autocommit = 0;` もできるが**この本は既定の `1` のまま `BEGIN` を書く方針**・**`TRUNCATE` / `ALTER TABLE` / `CREATE TABLE` / `DROP TABLE` はトランザクションの中でも即座に確定し、溜めていた変更まで確定させる**＝守れるのは `INSERT` / `UPDATE` / `DELETE` だけ）、**ACID**（原子性＝全部か無しか／一貫性＝型や制約を破った状態にならない／独立性＝途中の状態が他から見えない／永続性＝`COMMIT` したものは電源が落ちても残る。**いまは A と I が言えれば十分**・NoSQL はここを緩めているものがある＝第1章 1.5.2 の回収）、**ロック**（**2つの接続を開いて実演**（2つ目のターミナル＝PowerShell の新しいウィンドウ／`Command` + `T`／VS Code の `+`）・`BEGIN` 中の行は他の接続から書き換えられず**画面が止まる**・`COMMIT` で動き出し、待った秒数が `(N.NN sec)` に出る・**`stock = stock - 5` と書いたから正しく引ける**（`SELECT` で読んだ値を書き戻すと相手の変更が消える）・既定 50 秒で `ERROR 1205`＝**どこかに `BEGIN` したまま放置された接続がある**）、**デッドロック**（触る順番が逆＝互いの相手を待ち合う・`ERROR 1213` で**片方のトランザクション全体が `ROLLBACK` される**・**そのあとの `COMMIT` は `Query OK` でも何も確定しない**・バグではなくデータベースが正しく働いた結果なので、**やり直す**のが正解）、**避ける4つの習慣**（① `BEGIN` 〜 `COMMIT` を短く（**中で人間の入力を待たない**）／② **触る順番をそろえる**（`id` の小さい順）／③ `BEGIN` したまま閉じない（迷ったら `ROLLBACK;`）／④ 止まったら固まったのではなくロック待ちを疑う）、**この章はデータをわざと壊すので `SOURCE /sql/shop.sql;` で何度でも戻す** |
| 5 | テーブル設計 | 第4章の範囲に加えて：**練習は `shop` ではなく新しいデータベース `design`**（`CREATE DATABASE design CHARACTER SET utf8mb4;` → `USE design;`・題材は社内の貸出図書（`books` / `members` / `loans`）・`shop` に触るのは演習 5.4 だけ）、**データ型**（整数は `TINYINT` / `SMALLINT` / `INT` / `BIGINT`・**迷ったら `INT`**・範囲超過は `ERROR 1264`・`UNSIGNED` は使わない方針（0 以上は `CHECK` で書く）／**`VARCHAR(n)` の `n` は文字数**（`utf8mb4` なら日本語も1文字。2.6.1 の「バイト数」は文字コードが壊れているときの話）・超過は **切り捨てではなく `ERROR 1406`**・`CHAR(n)` は固定長（ISBN）・**`TEXT` は `DEFAULT` を持てない**（`ERROR 1101`）／`DATE` / `DATETIME` / `TIME`・**`TIMESTAMP` は使わない**（タイムゾーン変換・2038年）／**真偽値の専用型は無い。`BOOLEAN` は `TINYINT(1)` の別名で中身は `1`/`0`**（docker-text 6章で見た `done` の回収）／**金額に `FLOAT` / `DOUBLE` を使わない**（`1999.99` を入れても `= 1999.99` が偽・`* 3` が `5999.969970703125`・円は `INT`、小数が要るなら `DECIMAL(10, 2)`。3.6.3 の回収））、**制約**（`NOT NULL`（**`ERROR 1048`＝`NULL` を書いた／`ERROR 1364`＝列ごと省略した**・判断基準は「その列が空の行に意味があるか」・迷ったら付ける）／`DEFAULT`（**`DEFAULT CURRENT_TIMESTAMP`** で登録日時が自動・**既定値は既存の行には働かない**・`DESCRIBE` の `Default` 列で確認）／`UNIQUE`（`ERROR 1062`・**`NULL` は何行あってもよい**・**`UNIQUE (列A, 列B)` は組み合わせの重複だけを禁じる**）／`CHECK`（`ERROR 3819`・**`CONSTRAINT 名前` を付ける**（エラーに名前が出る）・列をまたぐ条件も書ける・**MySQL 8.0.16 未満では黙って無視される**）・**制約が働く順番の図**（型 → 既定値 → `NOT NULL` → `UNIQUE` → `CHECK` → 外部キー））、**主キー**（`PRIMARY KEY` は `NOT NULL` + `UNIQUE` を自動で持ち1テーブルに1つ・複合主キーは形だけ（第6章 6.7）・**主キーが無いと同じ内容の行の片方だけを消せない**／**`AUTO_INCREMENT` は欠番ができる**（配った番号は `ROLLBACK` でも `DELETE` でも戻らない・`LAST_INSERT_ID()`・`SHOW CREATE TABLE` の `AUTO_INCREMENT=N`・**欠番を埋め直さない**（`id` は順番ではなく名札）・`TRUNCATE` だけが 1 に戻す）／**自然キーと代理キー**（自然キーの弱点は「変わる・持たない行がある・長い」・**この本の方針＝主キーは `id` の代理キー、自然キーには `UNIQUE`**））、**外部キー**（`CONSTRAINT fk_子_親 FOREIGN KEY (列) REFERENCES 親 (列)`・**親テーブルと子テーブル**・型をそろえる（`ERROR 3780`）・親は主キーか `UNIQUE`・InnoDB・**索引が自動で張られる**（第7章）・`ALTER TABLE ... ADD CONSTRAINT` であとから足せるが**既存の行にも適用される**・**テーブルを作り直すと消える**（`SOURCE /sql/shop.sql;` のたびに消える）／**参照整合性**（`ERROR 1452`＝親がいない子を入れた・`ERROR 1451`＝子に指されている親を消した・**入れるのは親→子、消すのは子→親**（4.3.1 と `shop.sql` の `DROP` 順の回収）・外部キー列に `NOT NULL` を付けるかは別の判断（`NULL` は「未定」を表せる））／**`ON DELETE`**（既定の `RESTRICT`＝拒否 / `CASCADE`＝子も消える（**消えた子の行数は表示されない。迷ったら付けない**）/ `SET NULL`＝子の列が `NULL`・`shop` での使い分け（明細は `CASCADE`、顧客と分類は `RESTRICT`）・`ON UPDATE` は使わない））、**正規化**（1枚の台帳で起きる**3つの異常**（更新＝同じ本が2つの名前になる／挿入＝貸し出していない本を登録できない／削除＝最後の貸出を消すと本が消える）・**同じ事実は1か所にだけ置く**・第1正規形＝1マスに1値（`'A,B,C'` も `tag1`/`tag2`/`tag3` も不可。多対多は中間テーブルで第6章 6.7）・第2正規形＝複合主キーの一部で決まる列を追い出す・第3正規形＝主キー以外の列で決まる列を追い出す（`products` に `category_name` を持たない）・**例外＝「その時点の記録」**（`order_items.unit_price`・注文時の住所・契約時の税率。2.5.1 の回収）・**速さのための非正規化は測ってから**（第7章）・分けすぎの害）、**`ALTER TABLE`**（`ADD COLUMN` / `MODIFY COLUMN` / `CHANGE COLUMN`（改名）/ `DROP COLUMN` / `ADD CONSTRAINT` / `DROP CHECK` / `DROP FOREIGN KEY`・**`ROLLBACK` で戻せない**・**行があるテーブルに `NOT NULL` の列をいきなり足さない**（空文字や `0` が黙って入る。`DATE` では `ERROR 1292` のことも）→ **安全な3段階（`NULL` 可で足す → `UPDATE` で埋める → `MODIFY` で `NOT NULL`）**・**`MODIFY` は定義の書き直しなので書かなかった `DEFAULT` は消える**・短くすると `ERROR 1265`・改名は使っている SQL が全部 `ERROR 1054` になる・**あとから足す制約は既存の行にも適用される**／**本番の怖さ**（作り直しで時間がかかる・その間ほかの処理が待たされる・**列の入れ替えは5段階**（足す → 両方に書く → 移す → 新しい方を読む → 消す）・**手で打たずマイグレーションに任せる**（fastapi-text 6.6 の Alembic の回収）・打つ前のチェックリスト5項目）） |
...
> **注意：第3章の学習者が書けるのは `SELECT` だけです。**
> 第3章で扱ったのは `SELECT` / `FROM` / `WHERE` / `ORDER BY` / `LIMIT` / `OFFSET` /
> `DISTINCT` と、`LIKE` / `IN` / `BETWEEN` / `IS NULL` / `CASE` と、
> 文字列・日付・数値の関数です。
>
> - **`JOIN` は第6章です。** 「商品名と分類名を並べて出したい」という相談が来ても、
>   第3章の範囲では答えられません。`products.category_id` の**数値**までは出せる、
>   というところで止めてください
> - **集計（`COUNT` / `SUM` / `AVG` / `GROUP BY` / `HAVING`）は第6章 6.5 です。**
>   `SELECT COUNT(*) FROM 表名;` だけは 2.5.3 で「確認用の決まり文句」として
>   形を見せていますが、**`COUNT(列)` との違いや `GROUP BY` は扱っていません**。
>   「都道府県ごとの人数」のような相談は、第6章の内容だと伝えてください
> - **サブクエリは第6章 6.6 です。** 「いちばん高い商品」を出したいという相談には、
>   `ORDER BY price DESC LIMIT 1`（3.4.4）に着地させてください
> - **`INSERT` / `UPDATE` / `DELETE` は第4章です。** 第3章は**読むだけ**の章なので、
>   「試しても壊れない」ことを伝えて構いません。
>   データを直したいという相談には `SOURCE /sql/shop.sql;` を案内してください
> - **インデックス・`EXPLAIN` は第7章です。** 3.3.1 / 3.6.2 に「遅くなる」という
>   注記はありますが、**理由の説明も対処も第7章**です。先取りしないでください
> - 演習5問は、すべて `shop` の5テーブルだけで解けます。
>   **外部のデータやツールを使う案内はしないでください**
> - よくある相談と、最初に確認させるべきことは次のとおりです。
>
> | 症状 | 最初に疑うもの |
> |------|--------------|
> | 条件に合うはずなのに 0 件（`Empty set`） | **`= NULL` を書いていないか**（3.3.4）。`BETWEEN` の順番（3.3.3）、`LIKE` のつもりの `=`（3.3.1） |
> | `OR` を書いたら件数が多すぎる | **`AND` が先に処理されている**（3.2.4）。かっこを付ける |
> | 「〜以外」の件数が合わない | **`<>` で `NULL` の行が落ちている**（3.3.4）。`DESCRIBE` の `Null` 列を確認 |
> | `Unknown column 'x' in 'field list'` | `SELECT` 句の列名の打ち間違い（3.1.2） |
> | `Unknown column 'x' in 'where clause'` | **文字列の引用符忘れ**（3.2.1）、または **`WHERE` で別名を使った**（3.1.4 / 3.4.4） |
> | 並び順が実行するたびに変わる | **`ORDER BY` のキーが1つしかない**（3.4.3）。`id` を足す |
> | ページ送りで行が重複する・抜ける | 同上。`LIMIT` に `ORDER BY` が付いているかも確認（3.4.4） |
> | `CASE` のある分岐が一度も出ない | **`WHEN` の順番**（3.6.4）。狭い条件を先に書く |
> | ラベルの列が `NULL` になる | **`CASE` の `ELSE` 忘れ**（3.6.4） |
> | `ERROR 1064` | `;` の打ち忘れ（2.2.1）、`END` 忘れ（3.6.4）、`DISTINCT` の位置 |
>
> **「思ったのと違う結果が出る」はレベル A です**（第0章 0.2.2）。
> **答えの SQL を書かず、どの項を見直せばよいかだけを伝えてください。**
> 一方、`ERROR 1064` のような文法エラーの読み方は説明して構いません。

> **注意：第5章の学習者は、`design` という別のデータベースで練習しています。**
> 第5章の本文と演習 5.1 / 5.2 / 5.3 / 5.5 は、すべて `design` の中の
> `books` / `members` / `loans`（と演習で作る `magazines` / `equipment` / `employees` / `rentals`）
> に対するものです。**`shop` に触るのは演習 5.4 だけ**です。
> 「テーブルが無い」という相談を受けたら、まず `SELECT DATABASE();` を確認させてください。
>
> - **`JOIN` は第6章です。** 第5章で3つのテーブルに分けた直後なので、
>   「借りている人の名前つきで一覧を出したい」という相談が必ず来ます。
>   **`book_id` / `member_id` の数値までしか出せない**ところで止め、第6章だと伝えてください
> - **集計（`COUNT(列)` / `SUM` / `GROUP BY`）も第6章 6.5 です。**
>   `SELECT COUNT(*) FROM 表名;` だけは確認用の決まり文句として使えます（2.5.3）
> - **インデックスは第7章です。** 5.4.2 で「外部キーには索引が自動で張られる」と
>   触れていますが、`CREATE INDEX` も `EXPLAIN` も扱っていません。
>   「遅い」という相談は、**まず設計の話に着地させてから第7章へ**送ってください
> - **エラーは「決まりが働いた証拠」です。** `1048` / `1364` / `1062` / `3819` / `1452` / `1451` は、
>   どれも学習者が書いた制約が動いた結果です。**故障ではないことを最初に伝えてください**
> - **設計の相談（5.3.3 / 5.5.3）はレベル A です。** 正解が1つに決まりません。
>   **答えの `CREATE TABLE` を書かず、判断の材料**（何で決まる値か、あとから変わってよい値か）
>   を示してください
> - 演習 5.5 は分け方が解答例と違っても、
>   **「同じ文字列が複数行に現れていない」と「外部キーが2本ある」**を満たしていれば正解です
> - よくある相談と、最初に確認させるべきことは次のとおりです。
>
> | 症状 | 最初に疑うもの |
> |------|--------------|
> | `ERROR 1364` が出る | `NOT NULL` で `DEFAULT` の無い列を省略した（5.2.1）。`DESCRIBE` の `Default` 列 |
> | `ERROR 1062` が出る | `UNIQUE` の重複（5.2.3）。`for key` のあとの名前がどの制約か |
> | `ERROR 3819` が出る | `CHECK` の条件（5.2.4）。**制約の名前がメッセージに出ている** |
> | `ERROR 1452` が出る | 親の行がまだ無い（5.4.3）。**入れる順番は親 → 子** |
> | `ERROR 1451` が出る | 子が残っている（5.4.3）。**消す順番は子 → 親** |
> | 外部キーを足せない | **既存の行が矛盾している**（5.4.2 の注意）。先に `SELECT` で探す |
> | 外部キーがいつのまにか消えた | `SOURCE /sql/shop.sql;` を流した（5.4.2 の補足） |
> | `DEFAULT` が効かなくなった | **`MODIFY` で書き直して消した**（5.6.2 ②） |
> | `NOT NULL` にしたのに空の行がある | 行があるテーブルに後から足した（5.6.2 ①）。**空文字は `IS NULL` で探せない** |
> | `ERROR 1265` が出る | `MODIFY` で列を短くした（5.6.2 ④）。入っている値の長さを確認 |
