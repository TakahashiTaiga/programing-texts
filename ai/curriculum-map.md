...
| 章 | タイトル | 既習範囲（累積） |
|----|---------|----------------|
| 0 | はじめに（前提：docker-text 第2章まで＋fastapi-text 読了） | （SQL を1文も書かない章）この本の前提（react-text の `cd` / 拡張子の表示、docker-text の Docker Desktop 起動 / `stop` と `rm` / `compose.yaml` / `docker compose exec` / `down` と `down -v`、fastapi-text の `.env` / サーバー起動 / SQLAlchemy のモデルの戻り場所つき対応表）、`docker --version` / `docker compose version` による確認（**入っていないと第2章から先へ進めない**。`Cannot connect to the Docker daemon` は Docker Desktop 未起動）、**練習用 MySQL は docker-text 第6章の `fullstack-lesson` の `db` とは別に立てる**（別ディレクトリ＝プロジェクト名が違うのでボリュームも別。docker-text 5.2.4）、**空き容量 5 GB**（MySQL イメージ 600 MB 前後＋第7章で 10 万件投入）、**いま書ける SQL は `SHOW TABLES;` と `SELECT * FROM tasks;` の2行だけ**（docker-text 6.2.4 / 6.5.3）、**ゴールの形**（「先月いちばん売れた商品を上位3件」を `SELECT` / `INNER JOIN` / `SUM` / `GROUP BY` / `ORDER BY` / `LIMIT` で書いた15行を**読めなくてよい前提で**見せ、部分ごとに担当章を割り当てた表。**`products` / `orders` / `order_items` は説明用の仮のテーブル名で、実際の練習用テーブルは第2章 2.5 で決める**）、**この本の担当範囲**（ブラウザ ⇄ API は HTTP、API ⇄ データベースは SQL。右端の箱とそこへ送る SQL を扱う。Mermaid 図）、第8章で SQLite → MySQL とテーブル同士の関連、全5冊での位置づけ（5冊目・**フェーズ1の完走**）、章ごとに身につくこと（**山場は第2章（練習用データの確定）と第6章（`JOIN`）**）、AI サポートの準備（`ai/instructions.md` の読み込み・動作確認の2つの質問・章番号を添える意味）、**SQL は間違っていてもエラーを出さずに結果を返す**（`WHERE` 忘れの全件更新／`COUNT` の引数／`LEFT JOIN` + `WHERE`の3例を**章番号だけ予告**）→ **AI には「この SQL は何を返すか」を日本語で説明させる**／データが失われるかを実行前に確認させる、質問テンプレート（章番号／環境／**`DESCRIBE` の結果**／打った SQL／**期待した結果と実際の結果の両方**）と秘密情報（パスワード・実データの個人情報）を書き換えてから貼る、**この本のトラブルの2分類**（接続・文字化け・ポートは**レベル C**／「問いを SQL に翻訳する」は**レベル A**）、進め方（**読む向き（SQL→日本語）ではなく書く向き（日本語→SQL）だけが力になる**・確認の輪（問いを日本語で書く → SQL に翻訳して実行 → **件数を確認** → 中身を確認）・**0 件と全件はどちらも疑う**・第3章（取り出す）を第4章（変更する）より先に置いた理由＝取り出す操作は壊れない）、詰まったときの順番（**`mysql>` に戻っているか＝セミコロン忘れで `->`** → `USE` したか → テーブル名・列名のスペル → `ERROR` の行と番号 → 件数 → AI → 飛ばす。**第2章だけは飛ばせない**）、★の意味と解答編の使い方（その1＝第1章〜第5章／その2＝第6章〜第8章・**SQL は同じ結果になる書き方が複数あるので解答と違っても間違いとは限らない**（6.6.4／速度の違いは第7章）） |
...
| 9 | 次のステップ | フェーズ1の完走、これから学ぶこと |
| 解答編 | — | — |

> **注意**：**第0章の学習者は、MySQL をまだ起動していません。**
> 起動と接続は第2章です。第0章で打つコマンドは
> `docker --version` / `docker compose version` の2つだけで、
> **SQL は1文も書いていません。**
> 第0章の相談に、SQL を打たせる形で答えないでください。
>
> 第0章の学習者に対しては、次の点にも注意してください。
>
> - **0.1.2 に出てくる15行の SQL（`INNER JOIN` / `SUM` / `GROUP BY` を含むもの）は、
>   「読めなくてよいゴールの形」として見せているものです。**
>   質問されたら、部分ごとの担当章（第3章 / 第5章 / 第6章 / 第7章）を伝えるだけにして、
>   文法の説明を先取りしないでください
> - **0.1.2 の `products` / `orders` / `order_items` は説明用の仮のテーブル名**です。
>   この本で実際に使う練習用テーブルは**第2章 2.5 で決まります。**
>   仮のテーブル名を使った例題を作らないでください
> - 学習者が書ける SQL は、docker-text 6.2.4 / 6.5.3 で打った
>   **`SHOW TABLES;` と `SELECT * FROM tasks;` の2行だけ**という前提で答えてください
> - **練習用の MySQL は、docker-text 第6章の `fullstack-lesson` の `db` とは別に立てます**（0.1.1）。
>   「第6章の `db` に繋いで練習すればよい」と案内しないでください
> - 第2章の環境トラブル（接続できない・文字化け・ポート 3306 が使用中）は
>   **レベル C** です。手順を最後まで、コピペできる形で示してください。
>   一方で**「この問いを SQL でどう書くか」はレベル A** なので、完成した SQL は書かないでください
> - この本では、**SQL は間違っていてもエラーを出さずに結果を返す**ことを 0.2.2 で伝えています。
>   相談を受けたら、エラーメッセージだけでなく
>   **「期待した結果」と「実際の結果（とくに件数）」の両方**を確認してください
