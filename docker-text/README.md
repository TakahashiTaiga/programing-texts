...
理解度チェックと演習問題の解答・解説をまとめています（第1章〜第8章、全 83 問）。
...
| 項目 | 状態 |
|------|------|
| 本文 | ✅ 第0章〜第8章（9章・46 節・150 項） |
| 解答編 | ✅ 全 83 問（理解度チェック 52 問・演習 31 問）に解答と解説 |
| 図解 | ✅ Mermaid 62 箇所（本文 60・解答編 2）。SVG→PNG は不使用 |
| 通し確認（`D-FIN`） | ✅ 実施済み（下記） |
...
| 項目 | 結果 |
|------|------|
| README の章立てと本文の節・項の一致 | ✅ 過不足なし（9章・46 節・150 項。番号は `X.Y.Z` の3階層まで、`####` 以下は不使用） |
| 章間リンク・解答編リンク | ✅ 本の中の相対リンク 104 件・アンカー 83 件すべて到達可能 |
| 前章・次章のつながり | ✅ 第0章から第8章まで「次の章へ」が途切れず、最後は mysql-text に接続 |
| 理解度チェック・演習と解答の対応 | ✅ 全 83 問に解答があり、すべてに解説が付いている（演習 8.1 に解説を追記） |
| コードブロックの言語指定 | ✅ すべて指定あり。本文が `.svg` を直接参照している箇所もなし |
| Windows / macOS の手順 | ✅ 並記済み（7.2.2 に欠けていた `**Windows（PowerShell）**` の見出しを追加）。Windows 専用（WSL2）・macOS 専用（Apple Silicon）の節は、片方だけで正しい |
| `config.yaml` の `chapters` と実ファイル | ✅ 一致（10 ファイル） |
| 囲み記事のラベル | ✅ `docs/style-guide.md` の4種類（よくある間違い／補足／注意／つまずいたら）に統一（`重要：` の5箇所を `注意：` `補足：` に変更） |
| 用語の表記ゆれ・定義の重複 | ✅ 修正済み（デプロイ・ポーリングの重複定義に初出への参照を付与）。Adminer の画面文言「サーバ」「ユーザ名」は表示どおりで正しい（`style-guide.md` 2.1） |
| 解答編の見出し | ✅ 他の3冊と同じ `### 演習` に統一（8箇所を改名） |
| 用語集 | ✅ 未登録の 15 語を `docs/glossary.md` に追記 |

**人間の対応が残っているもの**（第2章の実機検証、イメージのバージョン確認など）は
...
| 日付 | 内容 |
|------|------|
| 2026-09 | 全9章＋解答編の執筆が完了。通し確認（`D-FIN`）を実施 |
